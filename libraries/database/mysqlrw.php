 ...
 * @package    Core
 * @author     wangzh
 * @version    $Id: mysqlrw.php 109 2011-06-05 07:00:30Z eric $
 * @copyright  (c) 2008-2009 Ko Team
 * @license    http://kophp.com/license.html
 */
class Database_MysqlRW extends Database_Driver
{

    /**
     * Database connection readonly links array
     */
    protected $_read_link = array();
    
    /**
     * Database connection write.
     */
    protected $_write_link;
    ...
    {
        is_resource($this->_write_link) && mysql_close($this->_write_link);
        foreach ($this->_read_link as $read_link) {
            is_resource($read_link) && mysql_close($read_link);
        }
    }
    ...
    public function connect ()
    {
        ...
        if (! is_resource($this->_write_link) || ! mysql_ping($this->_write_link))
            $this->_write_link = $this->_connnect($this->_config['write']);
            
        // Check read connection.
        $count = count($this->_config['read']);
        for ($i = 0; $i < $count; $i ++) {
            if (! isset($this->_read_link[$i]) || ! is_resource($this->_read_link[$i]) || ! mysql_ping($this->_read_link[$i])) {
                if (($link = $this->_connnect($this->_config['read'][$i])) !== FALSE) {
                    $this->_read_link[$i] = $link;
                } else {
                    unset($this->_read_link[$i]);
                }
            }
        }
        return FALSE;
    ...
    public function query ($sql)
    {
        ...
        // Whether is that 'SELECT' statement.
        if (preg_match("/^(\s*)select/i", $sql)) {
            $this->_link = $this->_read_link[array_rand($this->_read_link)];
        } else {
            $this->_link = $this->_write_link;
        }
    ...
    private function _connnect ($config)
    {
        // Persistent connections enabled?
        $connect = ($config['persistent'] == TRUE) ? 'mysql_pconnect' : 'mysql_connect';
        
        $host = isset($config['host']) ? $config['host'] : $config['socket'];
        $host .= isset($config['port']) ? ':' . $config['port'] : '';
        if (($this->_link = $connect($host, $config['username'], $config['password'], TRUE)) && mysql_select_db($this->_config['database'], $this->_link)) {
            $this->_config['charset'] && $this->charset($this->_config['charset']);
            $config['pass'] = NULL;
            return $this->_link;
        }
        return FALSE;
