 ...
 * @license    http://kophp.com/license
 */
class Remote
{
	...
	public static function get($url, array $options = NULL)
	{
		...
		if ( ! curl_setopt_array($remote, $options)) {
			throw new KoException('Failed to set CURL options, check CURL documentation: :url',
		...
		if (isset($error)) {
			throw new KoException('Error fetching remote :url [ status :code ] :error',
	...
	public static function status($url)
	{
		...
		fwrite($remote, 'User-Agent: Ko Framework (+http://kophp.com/)'.$CRLF);
